...
ModPlugFile* ModPlug_LoadPat(const void* data, int size)
{
	ModPlugFile* result = new ModPlugFile;
	ModPlug::UpdateSettings(true);
...
void ModPlug_UnloadPat(ModPlugFile* file)
{
...
int ModPlug_GetPatternNumberAtOrder(ModPlugFile* file, unsigned int ordidx) {
    return file->mSoundFile.GetPatternAtOrder(ordidx);
}
...
ModPlugNote* ModPlug_GetPattern(ModPlugFile* file,int pattern,unsigned int* numrows) {
	if ( pattern<MAX_PATTERNS && pattern >= 0) {
		if (file->mSoundFile.Patterns[pattern]) {
			if (numrows) {
                *numrows=(unsigned int)file->mSoundFile.PatternSize[pattern];
            }
//            
//            ModPlugNote *pats = (ModPlugNote*)file->mSoundFile.Patterns;
//            
//            int sizeOfPats = sizeof(pats);
//            int sizeOfNote = sizeof(ModPlugNote)
//            int numItems = sizeof(pats) / sizeof(ModPlugNote);
//            
//            printf("NUMBER ITEMS %i", numItems);
